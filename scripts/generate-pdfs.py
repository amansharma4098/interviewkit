...
fundamentals = dict(id='fundamentals', number='00', title='The Fundamentals', subtitle='Good answers start here.', experience='Every career stage', questions=12, color='mint', topics=['Core CS', 'Databases', 'Web essentials'])
...
def footer(canvas, doc, kit):
    if 3 <= doc.page <= 2 + (kit['questions'] + 1) // 2:
        canvas.setFillColor(colors.HexColor('#819575'))
        canvas.setFont('Helvetica-Bold', 8)
        canvas.drawString(48, 257, 'PRACTICE NOTES')
        canvas.setFont('Helvetica', 8)
        canvas.drawString(48, 242, 'An example, a trade-off, or a question to revisit.')
        canvas.setStrokeColor(colors.HexColor('#e1e8dc'))
        canvas.setLineWidth(.5)
        for y in range(217, 80, -27):
...
for kit in kits:
    ...
    assert len(entries) == kit['questions'], kit['id']
    ...
    for i, (section, question, answer, follow) in enumerate(entries):
        block = [Paragraph(f'{i + 1:02d} / {escape(section.upper())}', styles['section']), Paragraph(escape(question), styles['question']), Paragraph(escape(answer), styles['body']), Paragraph('<b>FOLLOW-UP</b><br/>' + escape(follow), styles['follow']), Spacer(1, 27)]
        story.append(KeepTogether(block))
        if (i + 1) % 2 == 0 and i + 1 < len(entries): story.append(PageBreak())
    story.extend([PageBreak(), Paragraph('Your practice worksheet.', styles['heading']), Paragraph('Choose one question from each section. Practice a 60-90 second explanation, then work through the follow-up. Record the specific gap you want to revisit.', styles['body']), Spacer(1, 20)])
    ...
    assert all(q[1] in text.replace('\n', ' ') for q in entries), f"Missing question in {kit['id']}"
